...
export default function useEditorRestApi() {
  const editorEndpoints = API_ENDPOINTS.editor;
  ...
  const findById = async (
    ...
    const { endpoint, method } = editorEndpoints.findById;
  ...
  const create = async (id: Countdown["id"]): Promise<APIResponse> => {
    const { endpoint, method } = editorEndpoints.create;
  ...
  const update = async (
    ...
    const { endpoint, method } = editorEndpoints.update;
  ...
  const remove = async (id: Countdown["id"]): Promise<APIResponse> => {
    const { endpoint, method } = editorEndpoints.delete;
    return await (
      await fetch(endpoint(id), {
        method: method,
        headers: {
